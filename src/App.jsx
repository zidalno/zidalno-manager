...
import { Search, Calculator, ChevronDown, Trophy, Flame, Wallet, RotateCw, ArrowRight, Users, Briefcase, ArrowUpRight, ArrowDownRight, AlertCircle, PlusCircle, X, BookOpen, ShieldCheck, TrendingUp, Banknote } from 'lucide-react';

// --- CONFIGURATION ---
const APP_CONFIG = {
  title: "ZIDALNO MANAGER",
  version: "Director's Cut v1.0"
};

// --- DONNÉES DE RÉFÉRENCE (DATABASE) ---
// Ces données servent à la fois pour le marché et pour identifier les ajouts au portefeuille
const ETF_DB = [
...
const PLAYERS_DATA = [
  { id: 1, ticker: 'MC', name: 'LVMH', price: 715, currency: '€', ovr: 91, position: 'LUXE', stats: { pac: 82, sho: 85, pas: 94, dri: 78, def: 88, phy: 70 }, broker: 'LCL (PEA)', country: 'FR', rarity: 'gold', comment: "Le patron du CAC40. Qualité 'Wide Moat' indétrônable." },
  { id: 2, ticker: 'ASML', name: 'ASML', price: 880, currency: '€', ovr: 94, position: 'TECH', stats: { pac: 96, sho: 45, pas: 99, dri: 90, def: 92, phy: 65 }, broker: 'IBKR (CTO)', country: 'NL', rarity: 'toty', comment: "Monopole mondial sur les machines EUV. Indispensable." },
  { id: 3, ticker: 'MSFT', name: 'MICROSOFT', price: 415, currency: '$', ovr: 93, position: 'TECH', stats: { pac: 88, sho: 60, pas: 98, dri: 85, def: 95, phy: 75 }, broker: 'IBKR (CTO)', country: 'US', rarity: 'icon', comment: "La valeur la plus solide du monde. Cloud + IA." },
  { id: 4, ticker: 'RMS', name: 'HERMÈS', price: 2200, currency: '€', ovr: 92, position: 'LUXE', stats: { pac: 85, sho: 40, pas: 99, dri: 88, def: 98, phy: 50 }, broker: 'LCL (PEA)', country: 'FR', rarity: 'gold', comment: "L'ultra-luxe. Pricing power absolu. Se paie cher." },
  { id: 5, ticker: 'KNSL', name: 'KINSALE', price: 380, currency: '$', ovr: 89, position: 'FIN', stats: { pac: 99, sho: 20, pas: 85, dri: 95, def: 90, phy: 80 }, broker: 'IBKR (CTO)', country: 'US', rarity: 'if', comment: "La pépite 'Momentum'. Croissance explosive." },
  { id: 6, ticker: 'OR', name: 'L\'ORÉAL', price: 445, currency: '€', ovr: 86, position: 'CONS', stats: { pac: 70, sho: 75, pas: 90, dri: 65, def: 85, phy: 72 }, broker: 'LCL (PEA)', country: 'FR', rarity: 'gold', comment: "Pilier défensif. Dividendes croissants réguliers." }
];

// Fusion pour la recherche d'ajout
...
const FutCard = ({ player }) => {
  ...
  const textColor = ['icon', 'gold'].includes(player.rarity) ? 'text-slate-900' : 'text-white';
  const labelColor = ['icon', 'gold'].includes(player.rarity) ? 'text-slate-700' : 'text-slate-300';
...
export default function ZidalnoManagerApp() {
  ...
  // --- ETAT DU PORTEFEUILLE (DYNAMIQUE) ---
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                <button 
                  ...
                  className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1 transition shadow-lg"
                >
              ...
              {portfolio.length === 0 ? (
                <div className="text-center py-10 border-2 border-dashed border-slate-700 rounded-xl text-slate-500 text-sm">
                  Aucun joueur dans l'équipe.<br/>Cliquez sur "Recruter" pour ajouter vos actions.
                </div>
              ) : (
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-left-4">
            <div className="bg-slate-800 border border-white/10 rounded-xl p-4 shadow-lg">
              <div className="flex items-center gap-2 mb-3 text-xs font-bold text-slate-300 uppercase tracking-widest border-b border-white/5 pb-2">
                <BookOpen size={14} className="text-blue-400" /> Centre de Formation (Stats)
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-900/50 p-2 rounded border border-white/5 flex items-start gap-2">
                  ...
                  <div>
                    <span className="text-[10px] font-black text-white block">CRO (Croissance/Pace)</span>
                    <p className="text-[9px] text-slate-400 leading-tight">Hausse du Chiffre d'Affaires et Cash Flow.</p>
                  </div>
                </div>
                <div className="bg-slate-900/50 p-2 rounded border border-white/5 flex items-start gap-2">
                  <Wallet size={14} className="text-yellow-400 mt-0.5 shrink-0" />
                  <div>
                    <span className="text-[10px] font-black text-white block">DIV (Dividende/Shoot)</span>
                    <p className="text-[9px] text-slate-400 leading-tight">Dividende stable ou en hausse régulière.</p>
                  </div>
                </div>
                <div className="bg-slate-900/50 p-2 rounded border border-white/5 flex items-start gap-2">
                  <ShieldCheck size={14} className="text-blue-400 mt-0.5 shrink-0" />
                  <div>
                    <span className="text-[10px] font-black text-white block">SUR (Sûreté/Pass)</span>
                    <p className="text-[9px] text-slate-400 leading-tight">Qualité du "Moat" (Avantage concurrentiel).</p>
                  </div>
                </div>
                <div className="bg-slate-900/50 p-2 rounded border border-white/5 flex items-start gap-2">
                  <Banknote size={14} className="text-purple-400 mt-0.5 shrink-0" />
                  <div>
                    <span className="text-[10px] font-black text-white block">VAL (Valeur/Physique)</span>
                    <p className="text-[9px] text-slate-400 leading-tight">Prix actuel vs Juste Valeur théorique.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-slate-900 border border-white/10 w-full max-w-sm rounded-2xl p-5 shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-black italic text-white flex items-center gap-2">
                <PlusCircle className="text-emerald-500" /> NOUVELLE RECRUE
              </h3>
