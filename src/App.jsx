...
export default function ZidalnoManagerApp() {
  ...
  const handleHardReset = () => { 
      if (confirm("Effacer tout le portefeuille ?")) { 
          localStorage.clear(); 
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < allTickers.length; i += chunkSize) {
        ...
        if(quotes) liveQuotes = [...liveQuotes, ...quotes];
      ...
      const mergedData = parsedPlayers.map(player => {
        ...
        // Fix variation erronée > 50%
        let liveChange = liveData ? (liveData.regularMarketChangePercent || liveData.changesPercentage || liveData.changePercent || 0) : 0;
        if (Math.abs(liveChange) > 50) liveChange = 0; // Sécurité bug Yahoo
        ...
        return { ...player, price: finalPrice, changePercent: finalChange, currency: currency, potential: potentialDisplay };
      });
